...
export const createPost = async (req, res) => {
  try {
    ...
    if(!user){
      return res.statust(404).json({message: "User not found"})
    }
    if(!text && !img){
...
export const commentOnPost = async (req, res) => { 
  try {
    ...
    console.error("Error in commenOnPost controller", error)
    res.status(500).json({error: "Internal server Error"})
  }
}
...
export const deletePost = async (req, res) => {
  try {
    ...
    if(post.user.toString() !== req.user._id.toString()){
      res.status(401).json({error: "You are not authorize to delete this post"})
    }
    if(post.img){
